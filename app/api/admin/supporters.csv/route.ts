...
export async function GET() {
  ...
  const content = [
    ["Nome", "Telefone", "Cidade", "Bairro", "Interesse", "Status", "Origem", "Dispositivo", "Visualizações", "Sessões", "Primeiro acesso", "Último acesso", "Data do cadastro"],
    ...rows.map((r) => [r.name, r.phone, r.city, r.neighborhood, r.interest, r.status, r.access_source || r.utm_source || r.referrer || "Direto", r.device_type || "", r.access_count, r.session_count, r.first_access_at ? new Date(r.first_access_at).toISOString() : "", r.last_access_at ? new Date(r.last_access_at).toISOString() : "", new Date(r.created_at).toISOString()]),
