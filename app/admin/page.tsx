...
export default async function AdminPage() {
  ...
  return <main className="adminShell">
    ...
    <section className="adminIntro"><div><p>Painel administrativo</p><h1>Cadastros do site</h1><span>Dados protegidos e centralizados para acompanhamento da equipe.</span></div><a className="adminExport" href="/api/admin/supporters.csv">Exportar CSV ↓</a></section>
    ...
    <section className="adminTableWrap"><div className="adminTableHead"><h2>Contatos mais recentes</h2><span>Até 500 registros</span></div>
      {supporters.length ? <div className="adminTableScroll"><table><thead><tr><th>Nome</th><th>Telefone</th><th>Localidade</th><th>Interesse</th><th>Data</th></tr></thead><tbody>{supporters.map((item) => <tr key={item.id}><td><strong>{item.name}</strong><small>{item.status}</small></td><td>{item.phone}</td><td>{item.city}<small>{item.neighborhood || "—"}</small></td><td>{interestLabels[item.interest] || item.interest}</td><td>{new Date(item.created_at).toLocaleDateString("pt-BR")}</td></tr>)}</tbody></table></div> : <div className="adminEmpty"><strong>Nenhum cadastro ainda.</strong><p>Os novos contatos aparecerão aqui depois do envio do formulário.</p></div>}
    </section>
  </main>;
}
